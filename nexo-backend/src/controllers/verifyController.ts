...
export async function verifyByExpediente(expediente: string) {
  try {
    ...
    // 7. Verificar que esté certificado por ONCE
    const isValid = personalData.verificadoONCE === true

    // 8. Retornar verificación
    return {
      success: true,
      verified: isValid,
      source: 'blockchain',
